...
test('Octicons have svg', (t) => {
  ...
  for (const point of Object.keys(octicons)) {
    t.truthy(
      octicons[point].toSVG(),
      `The octicon "${point}" doesn't have svg`
    );
  }
});
...
test('Octicons have default html attributes', (t) => {
  ...
  for (const point of Object.keys(octicons)) {
    ...
    t.regex(
      svg,
      /version="1\.1"/,
      `The octicon "${point}" doesn't have the version attribute`
    );
    t.regex(
      svg,
      /aria-hidden="true"/,
      `The octicon "${point}" doesn't have the aria-hidden attribute`
    );
    t.regex(
      svg,
      new RegExp(`width=`),
...
test('Passing in classnames will be included in output', (t) => {
  ...
  for (const point of Object.keys(octicons)) {
    ...
    t.regex(
      svg,
      new RegExp(
        `class="octicon octicon-${octicons[point].symbol} new-class another-class"`
      ),
      `The octicon "${point}" doesn't have the class attribute`
    );
  }
...
test('Passing in width will size properly', (t) => {
  ...
  t.regex(
    svg,
    new RegExp('width="60"'),
    'The octicon "x" doesn\'t have the width attribute scaled properly'
  );
});
...
test('Passing in height will size properly', (t) => {
  ...
  t.regex(
    svg,
    new RegExp('height="45"'),
    'The octicon "x" doesn\'t have the height attribute scaled properly'
  );
});
